...
export default function ROI() {
  ...
  return (
    <section id="roi" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
        <div className="rounded-3xl border border-gray-200 bg-white p-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">
              ROI & impact (illustrative)
            </h3>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <MetricCard
              ...
              value={currency(monthlyProductivityValue)}
            />
            <MetricCard
              title="Monthly turnover savings"
              value={currency(monthlyTurnoverSavings)}
            />
            <MetricCard
              title="Estimated monthly impact"
              value={currency(monthlyImpact)}
              accent
            />
            <MetricCard
              title="Employer cash cost"
              value="~ KSh 0 (fees paid by employees)"
            />
          </div>
          <p className="mt-3 text-xs text-gray-600">
            Fees shown in-app to employees before confirmation: Flat KSh 25 + 5%
            application; no interest. Configurable employer policies ensure
            responsible usage.
          </p>
        </div>
        ...
        <div>
          <h3 className="text-2xl font-semibold text-gray-900">
            A wellbeing benefit with measurable outcomes
          </h3>
          <ul className="mt-4 space-y-3 text-gray-700">
            <li className="flex items-start gap-2">
              <CheckCircle2 className="h-5 w-5" style={{ color: BRAND }} />
              <span>
                Lower absenteeism and overtime leakage through better cash-flow
                timing.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="h-5 w-5" style={{ color: BRAND }} />
              <span>
                Higher engagement & retention, especially in shift-based and
                frontline teams.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="h-5 w-5" style={{ color: BRAND }} />
              <span>
                No lending exposure to the employer; settlement auto-reconciles
                in payroll.
              </span>
            </li>
          </ul>
