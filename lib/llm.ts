...
export async function generateNoahResponse(opts: {
  ...
  const model =
    opts.model ?? (process.env.NOAH_DEFAULT_MODEL as ModelName) ?? "openai";
  ...
  if (model === "openai") {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are Noah, EaziWage AI assistant. Be concise and professional in your responses. Do not reveal PII.",
        },
    ...
    const text =
      data?.generated_text ?? data?.results?.[0]?.text ?? JSON.stringify(data);
